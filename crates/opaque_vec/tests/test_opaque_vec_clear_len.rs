...
fn run_test_opaque_vec_clear_len<T>(values: &[T])
where
...
fn run_test_opaque_vec_clear_len_values<T>(values: &[T])
where
    ...
    <T as TryFrom<usize>>::Error: fmt::Debug,
{
    for len in 0..values.len() {
        ...
        run_test_opaque_vec_clear_len(prefix_values);
    }
}
