...
fn run_test_opaque_vec_shift_insert_contains<T>(values: &[T])
where
    T: any::Any + PartialEq + Clone + fmt::Debug
{
    ...
    for (i, value) in values.iter().cloned().enumerate() {
        vec.shift_insert::<T, alloc::Global>(i, value);
    }
...
fn run_test_opaque_vec_shift_insert_contains_values<T>(values: &[T])
where
    T: any::Any + PartialEq + Clone + fmt::Debug
{
    let iter = ovt::PrefixGenerator::new(values);
    for slice in iter {
        run_test_opaque_vec_shift_insert_contains(slice);
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_opaque_vec_shift_insert_contains_range_values() {
                let values = opaque_vec_testing::range_values::<$typ, $max_array_size>($range_spec);
                run_test_opaque_vec_shift_insert_contains_values(&values);
            }

            #[test]
            fn test_opaque_vec_shift_insert_contains_alternating_values() {
