...
fn run_test_opaque_vec_push_get<T, A>(values: &[T], alloc: A)
where
...
fn run_test_opaque_vec_push_get_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iterator {
        run_test_opaque_vec_push_get(slice, alloc.clone());
    }
}
