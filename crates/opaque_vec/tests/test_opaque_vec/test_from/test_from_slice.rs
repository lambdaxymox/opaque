    ...
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
{
    ...
    for slice in iterator {
        run_test_opaque_vec_from_slice(slice);
    }
}
    ...
    ($module_name:ident, $typ:ty, $max_array_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_vec_from_slice_empty() {
                let values: [$typ; 0] = [];
                let vec = OpaqueVec::from(values.clone());
            ...
            #[test]
            fn test_opaque_vec_from_slice_range_values() {
                let values = opaque_vec_testing::range_values::<$typ, $max_array_size>($range_spec);
                run_test_opaque_vec_from_slice_values(&values);
            }

            #[test]
            fn test_opaque_vec_from_slice_alternating_values() {
