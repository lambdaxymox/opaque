...
macro_rules! generate_props {
    ($module_name:ident, $typ:ty, $max_length:expr, $vec_gen:ident) => {
        mod $module_name {
            ...
            proptest! {
                #[test]
                fn prop_replace_insert_contains(values in super::$vec_gen::<$typ, alloc::Global>($max_length)) {
                    let values: super::OpaqueVec = values;
                    super::prop_replace_insert_contains::<$typ, alloc::Global>(values)?
                }

                #[test]
                fn prop_replace_insert_get(values in super::$vec_gen::<$typ, alloc::Global>($max_length)) {
                    let values: super::OpaqueVec = values;
                    super::prop_replace_insert_get::<$typ, alloc::Global>(values)?
                }

                #[test]
                fn prop_replace_insert_len(values in super::$vec_gen::<$typ, alloc::Global>($max_length)) {
                    let values: super::OpaqueVec = values;
                    super::prop_replace_insert_len::<$typ, alloc::Global>(values)?
                }
            }
        }
    };
