...
fn prop_from_iter_slice<T, A>(values: TypeErasedVec) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    let result_values = TypeErasedVec::from_iter(values.as_slice::<T, A>().iter().cloned());
...
macro_rules! generate_props {
    ($module_name:ident, $typ:ty, $alloc_typ:ty, $max_length:expr, $vec_gen:ident) => {
        mod $module_name {
            ...
            proptest! {
                #[test]
                fn prop_from_iter_slice(values in super::$vec_gen::<$typ, $alloc_typ>($max_length)) {
                    let values: super::TypeErasedVec = values;
                    super::prop_from_iter_slice::<$typ, $alloc_typ>(values)?
                }
            }
        }
    };
