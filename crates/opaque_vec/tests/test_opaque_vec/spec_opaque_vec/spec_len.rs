...
fn prop_len<T, A>(values: TypeErasedVec) -> Result<(), TestCaseError>
where
...
macro_rules! generate_props {
    ($module_name:ident, $typ:ty, $alloc_typ:ty, $max_length:expr, $vec_gen:ident) => {
        mod $module_name {
            ...
            proptest! {
                #[test]
                fn prop_len(values in super::$vec_gen::<$typ, $alloc_typ>($max_length)) {
                    let values: super::TypeErasedVec = values;
                    super::prop_len::<$typ, $alloc_typ>(values)?
                }
            }
        }
    };
