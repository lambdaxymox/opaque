...
fn run_test_opaque_vec_drain_nothing_vec<T, A>(values: &[T], start: usize, alloc: A)
where
...
fn run_test_opaque_vec_drain_nothing_vec_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iterator {
        for i in 0..slice.len() {
            run_test_opaque_vec_drain_nothing_vec(slice, i, alloc.clone());
        }
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, $typ:ty, $max_vec_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_opaque_vec_drain_nothing_vec_empty() {
                let values: [$typ; 0] = [];
                let alloc = alloc::Global;
                run_test_opaque_vec_drain_nothing_vec_values(&values, alloc);
            }

            #[test]
            fn test_opaque_vec_drain_nothing_vec_range_values() {
            ...
            #[test]
            fn test_opaque_vec_drain_nothing_vec_alternating_values() {
