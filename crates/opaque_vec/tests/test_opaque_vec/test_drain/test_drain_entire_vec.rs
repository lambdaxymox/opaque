...
fn run_test_typed_proj_vec_drain_entire_vec<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let mut vec = common::opaque_vec::from_slice_in(values, alloc.clone());
    let expected = vec.clone::<T, A>();
    let result = {
        let mut _result = OpaqueVec::new_in::<T, A>(alloc.clone());
        for value in vec.drain::<_, T, A>(..) {
...
fn run_test_typed_proj_vec_drain_entire_vec_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iter {
        run_test_typed_proj_vec_drain_entire_vec(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_vec_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_typed_proj_vec_drain_entire_vec_empty() {
                let values: [$typ; 0] = [];
                let alloc = alloc::Global;
                run_test_typed_proj_vec_drain_entire_vec(&values, alloc);
            }

            #[test]
            fn test_typed_proj_vec_drain_entire_vec_range_values() {
                let values = opaque_vec_testing::range_values::<$typ, $max_vec_size>($range_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_vec_drain_entire_vec_values(&values, alloc);
            }

            #[test]
            fn test_typed_proj_vec_drain_entire_vec_alternating_values() {
