...
fn run_test_opaque_vec_shift_insert_shift_remove<T, A>(values: &[T], alloc: A, new_value: T)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let expected = common::opaque_vec::from_slice_in(values, alloc.clone());
    let mut result = common::opaque_vec::from_slice_in(values, alloc.clone());
...
fn run_test_opaque_vec_shift_insert_shift_remove_values<T, A>(values: &[T], alloc: A, new_value: T)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iter {
        run_test_opaque_vec_shift_insert_shift_remove(slice, alloc.clone(), new_value.clone());
    }
}

macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $new_value:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $typ {
            use super::*;

            #[test]
            fn test_opaque_vec_shift_insert_shift_remove_range_values() {
                let values = opaque_vec_testing::range_values::<$typ, $max_array_size>($range_spec);
                let alloc = alloc::Global;
                let new_value: $typ = $new_value;
                run_test_opaque_vec_shift_insert_shift_remove_values(&values, alloc, new_value);
            }

            #[test]
            fn test_opaque_vec_shift_insert_shift_remove_alternating_values() {
