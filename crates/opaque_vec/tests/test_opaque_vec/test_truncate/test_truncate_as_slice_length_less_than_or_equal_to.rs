...
fn run_test_opaque_vec_truncate_as_slice_length_less_than_or_equal_to<T, A>(values: &[T], alloc: A)
where
...
fn run_test_opaque_vec_truncate_as_slice_length_less_than_or_equal_to_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    ($module_name:ident, $typ:ty, $max_array_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_vec_truncate_as_slice_length_less_than_or_equal_to_empty() {
                ...
                run_test_opaque_vec_truncate_as_slice_length_less_than_or_equal_to_values(&values, alloc);
            }
