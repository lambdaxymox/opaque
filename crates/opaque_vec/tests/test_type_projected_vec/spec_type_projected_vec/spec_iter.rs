...
fn strategy_prop_iter_take<T, A>(max_length: usize) -> impl Strategy<Value = (TypeProjectedVec<T, A>, usize)>
where
...
fn prop_iter_back_to_vec<T, A>(values: TypeProjectedVec<T, A>) -> Result<(), TestCaseError>
where
...
fn prop_iter_take<T, A>((values, count): (TypeProjectedVec<T, A>, usize)) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    ...
    let mut result = TypeProjectedVec::new_proj_in(values.allocator().clone());
    result.extend(values.iter().cloned().take(count));
...
fn prop_iter_take_none<T, A>(values: TypeProjectedVec<T, A>) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    ...
    let mut result = TypeProjectedVec::new_proj_in(values.allocator().clone());
    result.extend(values.iter().cloned().take(0));
...
fn prop_iter_ordering<T, A>(values: TypeProjectedVec<T, A>) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_iter_back_to_vec(values in super::$vec_gen::<$typ, $alloc_typ>($max_length)) {
                    ...
                    super::prop_iter_back_to_vec(values)?
                }
