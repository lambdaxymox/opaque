...
fn run_test_type_projected_vec_into_iter_drop<A>(length: usize, alloc: A)
where
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
...
fn run_test_type_projected_vec_into_iter_take_then_drop<A>(length: usize, take_count: usize, alloc: A)
where
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let (drop_counter, vec) = create_drop_counter_vec_in(length, alloc.clone());
    let mut taken_vec = TypeProjectedVec::with_capacity_in(take_count, alloc.clone());
    {
...
fn test_type_projected_vec_into_iter_drop() {
    ...
    for length in 0..max_length {
        for take_count in 0..length {
            run_test_type_projected_vec_into_iter_drop(length, alloc.clone());
        }
    }
}
...
fn test_type_projected_vec_into_iter_take_then_drop() {
    ...
    for length in 0..max_length {
        for take_count in 0..length {
            run_test_type_projected_vec_into_iter_take_then_drop(length, take_count, alloc.clone());
        }
    }
}
