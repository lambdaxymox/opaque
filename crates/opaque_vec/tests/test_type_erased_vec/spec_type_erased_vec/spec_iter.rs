...
fn strategy_prop_iter_take<T, A>(max_length: usize) -> impl Strategy<Value = (TypeErasedVec, usize)>
where
...
fn prop_iter_back_to_vec<T, A>(values: TypeErasedVec) -> Result<(), TestCaseError>
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    ...
    for value in vec.iter::<T, A>().cloned() {
...
fn prop_iter_take<T, A>((values, count): (TypeErasedVec, usize)) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    ...
    let mut result = TypeErasedVec::new_proj_in::<T, A>(values.allocator::<T, A>().clone());
    result.extend::<_, T, A>(values.iter::<T, A>().cloned().take(count));
...
fn prop_iter_take_none<T, A>(values: TypeErasedVec) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    ...
    let mut result = TypeErasedVec::new_proj_in::<T, A>(values.allocator::<T, A>().clone());
    result.extend::<_, T, A>(values.iter::<T, A>().cloned().take(0));
...
fn prop_iter_ordering<T, A>(values: TypeErasedVec) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    let vec = values.clone::<T, A>();
    let mut iter = vec.iter::<T, A>();
    for i in 0..vec.len() {
        let expected = vec.get::<_, T, A>(i);
...
macro_rules! generate_props {
    ($module_name:ident, $typ:ty, $alloc_typ:ty, $max_length:expr, $vec_gen:ident) => {
        mod $module_name {
            ...
            proptest! {
                #[test]
                fn prop_iter_back_to_vec(values in super::$vec_gen::<$typ, $alloc_typ>($max_length)) {
                    let values: super::TypeErasedVec = values;
                    super::prop_iter_back_to_vec::<$typ, $alloc_typ>(values)?
                }

                #[test]
                fn prop_iter_take(values in super::strategy_prop_iter_take::<$typ, $alloc_typ>($max_length)) {
                    let values: (super::TypeErasedVec, usize) = values;
                    super::prop_iter_take::<$typ, $alloc_typ>(values)?
                }

                #[test]
                fn prop_iter_take_none(values in super::$vec_gen::<$typ, $alloc_typ>($max_length)) {
                    let values: super::TypeErasedVec = values;
                    super::prop_iter_take_none::<$typ, $alloc_typ>(values)?
                }

                #[test]
                fn prop_iter_ordering(values in super::$vec_gen::<$typ, $alloc_typ>($max_length)) {
                    let values: super::TypeErasedVec = values;
                    super::prop_iter_ordering::<$typ, $alloc_typ>(values)?
                }
            }
        }
    };
