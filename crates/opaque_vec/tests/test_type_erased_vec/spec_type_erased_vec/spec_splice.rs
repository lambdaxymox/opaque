    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    (1..=max_length, 0..=max_splice_length).prop_flat_map(move |(length, splice_length)| {
        let vec_gen = strategy_type_erased_vec_len::<T, A>(length);
        let range_gen = strategy_vec_range(length);
        let splice_vec_gen = strategy_type_erased_vec_len::<T, A>(splice_length);
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let expected = {
        ...
        vec.drain::<_, T, A>(range.clone());
        for i in 0..splice_values.len() {
            vec.shift_insert::<T, A>(range.start + i, splice_values.as_slice::<T, A>()[i].clone());
        }
    ...
    let result = {
        let mut vec = values.clone::<T, A>();
        vec.splice::<_, _, T, A>(range.clone(), splice_values.as_slice::<T, A>().iter().cloned());
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_splice_equivalent_to_drain_and_shift_insert(
                    ...
                    let splice_values: super::TypeErasedVec = splice_values;
                    super::prop_splice_equivalent_to_drain_and_shift_insert::<$typ, $alloc_typ>(values, range, splice_values)?
                }
                ...
                fn prop_splice_len(
                    ...
                    let range: super::ops::Range<usize> = range;
                    let splice_values: super::TypeErasedVec = splice_values;
                    super::prop_splice_len::<$typ, $alloc_typ>(values, range, splice_values)?
                }
            }
        }
    };
