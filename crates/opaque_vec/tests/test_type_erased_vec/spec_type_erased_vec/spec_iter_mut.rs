...
fn strategy_prop_iter_mut_take<T, A>(max_length: usize) -> impl Strategy<Value = (TypeErasedVec, usize)>
where
...
fn prop_iter_mut_back_to_vec<T, A>(values: TypeErasedVec) -> Result<(), TestCaseError>
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    ...
    for value in vec.iter_mut::<T, A>() {
        result.push::<T, A>(value.clone());
    }
...
fn prop_iter_mut_take<T, A>((values, count): (TypeErasedVec, usize)) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    ...
    let mut result = TypeErasedVec::new_proj_in::<T, A>(values.allocator::<T, A>().clone());
    result.extend::<_, T, A>(vec.iter_mut::<T, A>().map(|v| v.clone()).take(count));
...
fn prop_iter_mut_take_none<T, A>(values: TypeErasedVec) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    ...
    let mut result = TypeErasedVec::new_proj_in::<T, A>(values.allocator::<T, A>().clone());
    result.extend::<_, T, A>(vec.iter_mut::<T, A>().map(|v| v.clone()).take(0));
...
fn prop_iter_mut_ordering<T, A>(values: TypeErasedVec) -> Result<(), TestCaseError>
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default + fmt::Debug,
{
    let mut vec = values.clone::<T, A>();
    let mut iter = vec.iter_mut::<T, A>();
    for i in 0..values.len() {
        let expected = values.get::<_, T, A>(i).cloned();
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_iter_mut_back_to_vec(values in super::$vec_gen::<$typ, $alloc_typ>($max_length)) {
                    ...
                    super::prop_iter_mut_back_to_vec::<$typ, $alloc_typ>(values)?
                }
