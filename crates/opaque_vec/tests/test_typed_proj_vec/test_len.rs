...
fn run_test_typed_proj_vec_len<T, A>(values: &[T], alloc: A)
where
...
fn run_test_typed_proj_vec_len_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone,
    A: any::Any + alloc::Allocator + Clone,
{
    ...
    for slice in iter {
        run_test_typed_proj_vec_len(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_typed_proj_vec_len_range_values() {
                let values = opaque_vec_testing::range_values::<$typ, $max_array_size>($range_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_vec_len_values(&values, alloc);
            }

            #[test]
            fn test_typed_proj_vec_len_alternating_values() {
