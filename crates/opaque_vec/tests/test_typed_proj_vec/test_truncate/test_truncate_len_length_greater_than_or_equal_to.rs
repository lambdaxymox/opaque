...
fn run_test_typed_proj_vec_truncate_len_length_greater_than_or_equal_to<T, A>(values: &[T], alloc: A)
where
...
fn run_test_typed_proj_vec_truncate_len_length_greater_than_or_equal_to_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iterator {
        run_test_typed_proj_vec_truncate_len_length_greater_than_or_equal_to(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, $typ:ty, $max_array_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_typed_proj_vec_truncate_len_length_greater_than_or_equal_to_empty() {
                let values: [$typ; 0] = [];
                let alloc = alloc::Global;
                run_test_typed_proj_vec_truncate_len_length_greater_than_or_equal_to(&values, alloc);
            }

            #[test]
            fn test_typed_proj_vec_truncate_len_length_greater_than_or_equal_to_range_values() {
                let values = ovt::range_values::<$typ, $max_array_size>($range_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_vec_truncate_len_length_greater_than_or_equal_to_values(&values, alloc);
            }

            #[test]
            fn test_typed_proj_vec_truncate_len_length_greater_than_or_equal_to_alternating_values() {
