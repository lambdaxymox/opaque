...
fn run_test_typed_proj_vec_shift_remove_len<T, A>(values: &[T], alloc: A)
where
...
fn run_test_typed_proj_vec_shift_remove_len_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iter {
        run_test_typed_proj_vec_shift_remove_len(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, $typ:ty, $max_array_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_typed_proj_vec_shift_remove_len_range_values() {
                let values = opaque_vec_testing::range_values::<$typ, $max_array_size>($range_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_vec_shift_remove_len_values(&values, alloc);
            }

            #[test]
            fn test_typed_proj_vec_shift_remove_len_alternating_values() {
