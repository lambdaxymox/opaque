...
fn run_test_typed_proj_vec_append_len_source<T, A>(values1: &[T], values2: &[T], alloc: A)
where
...
fn run_test_typed_proj_vec_append_len_destination<T, A>(values1: &[T], values2: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
...
fn run_test_typed_proj_vec_append_len_source_values<T, A>(values1: &[T], values2: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        ...
        for destination in iterator2 {
            run_test_typed_proj_vec_append_len_source::<T, A>(source, destination, alloc.clone());
        }
    }
}

fn run_test_typed_proj_vec_append_len_destination_values<T, A>(values1: &[T], values2: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        let iterator2 = ovt::PrefixGenerator::new(values2);
        for destination in iterator2 {
            run_test_typed_proj_vec_append_len_destination::<T, A>(source, destination, alloc.clone());
        }
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, $typ:ty, $max_array_size:expr, $src_range_spec:expr, $src_alt_spec:expr, $dst_range_spec:expr, $dst_alt_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn run_test_typed_proj_vec_append_len_source_empty() {
                let values1: [$typ; 0] = [];
                let values2: [$typ; 0] = [];
                let alloc = alloc::Global;
                run_test_typed_proj_vec_append_len_source_values(&values1, &values2, alloc);
            }

            #[test]
            fn test_typed_proj_vec_append_len_source_range_values() {
            ...
            #[test]
            fn test_typed_proj_vec_append_len_source_alternating_values() {
            ...
            #[test]
            fn run_test_typed_proj_vec_append_len_destination_empty() {
                let values1: [$typ; 0] = [];
                let values2: [$typ; 0] = [];
                let alloc = alloc::Global;
                run_test_typed_proj_vec_append_len_destination_values(&values1, &values2, alloc);
            }

            #[test]
            fn test_typed_proj_vec_append_len_destination_range_values() {
                let values1 = ovt::range_values::<$typ, $max_array_size>($src_range_spec);
                let values2 = ovt::range_values::<$typ, $max_array_size>($dst_range_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_vec_append_len_destination_values(&values1, &values2, alloc);
            ...
            #[test]
            fn test_typed_proj_vec_append_len_destination_alternating_values() {
                let values1 = ovt::alternating_values::<$typ, $max_array_size>($src_alt_spec);
                let values2 = ovt::alternating_values::<$typ, $max_array_size>($dst_alt_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_vec_append_len_destination_values(&values1, &values2, alloc);
