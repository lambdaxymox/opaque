...
fn run_test_typed_proj_vec_from_iter_slice<T>(expected: &[T])
where
...
fn run_test_typed_proj_vec_from_iter_slice_values<T>(values: &[T])
where
    T: any::Any + PartialEq + Clone + Default + fmt::Debug,
{
...
macro_rules! generate_tests {
    ($module_name:ident, $typ:ty, $max_array_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_typed_proj_vec_from_iter_slice_range_values() {
                let values = opaque_vec_testing::range_values::<$typ, $max_array_size>($range_spec);
                run_test_typed_proj_vec_from_iter_slice_values(&values);
            }

            #[test]
            fn test_typed_proj_vec_from_iter_slice_alternating_values() {
