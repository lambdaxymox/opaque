...
fn run_test_typed_proj_vec_swap_remove_start<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Clone,
{
    ...
    for i in 0..values.len() {
        ...
        let new_vec = expected(&values[0..last_index], alloc.clone());
...
fn run_test_typed_proj_vec_swap_remove_start_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Clone,
{
    ...
    for slice in iter {
        run_test_typed_proj_vec_swap_remove_start(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $range_spec:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_typed_proj_vec_swap_remove_start_range_values() {
                let values = opaque_vec_testing::range_values::<$typ, $max_array_size>($range_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_vec_swap_remove_start_values(&values, alloc);
            }

            #[test]
            fn test_typed_proj_vec_swap_remove_start_alternating_values() {
