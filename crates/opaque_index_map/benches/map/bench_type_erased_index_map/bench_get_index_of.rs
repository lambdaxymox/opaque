...
fn bench_type_erased_index_map_get_index_of(c: &mut Criterion) {
    let keys = 0..1000;
    let values = 1..1001;
    ...
    c.bench_function("opaque_index_map_get_index_of", |b| {
        b.iter(|| {
            for key in opaque_map.keys::<i32, i32, hash::RandomState, alloc::Global>() {
                let _ = core::hint::black_box(opaque_map.get_index_of::<i32, i32, i32, hash::RandomState, alloc::Global>(key));
            }
        });
    });
}

criterion_group!(bench_get_index_of, bench_type_erased_index_map_get_index_of, bench_index_map_get_index_of);
