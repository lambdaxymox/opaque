...
fn bench_type_projected_index_map_shift_insert(c: &mut Criterion) {
    c.bench_function("type_projected_index_map_shift_insert", |b| {
        b.iter(|| {
            let keys = 0..1000;
            let values = 1..1001;
...
criterion_group!(
    bench_shift_insert,
    bench_type_projected_index_map_shift_insert,
