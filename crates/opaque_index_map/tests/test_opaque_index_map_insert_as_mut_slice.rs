...
fn run_test_opaque_index_map_insert_as_mut_slice_values<K, V>(generator: opaque_testing::PrefixGenerator<K, V>)
where
    K: Clone + Eq + hash::Hash + fmt::Debug + 'static,
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_gen = $range_gen:expr, const_gen = $const_gen:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_as_mut_slice_empty() {
                ...
                let mut generator = opaque_testing::key_value_pairs(keys_vec.iter::<$key_typ>().cloned(), values_vec.iter::<$value_typ>().cloned());
