...
fn run_test_opaque_index_map_swap_remove_full_get_full_mut<K, V>(entries: &[(K, V)])
where
    ...
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    for key in keys.iter() {
        ...
        let result = map.swap_remove_full::<K, K, V>(key);

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_map_swap_remove_full_get_full_mut_values<K, V>(entries: &[(K, V)])
where
    K: Clone + Eq + hash::Hash + fmt::Debug + 'static,
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_swap_remove_full_get_full_mut(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_swap_remove_full_get_full_mut_empty() {
                ...
                run_test_opaque_index_map_swap_remove_full_get_full_mut_values(&entries);
            }
