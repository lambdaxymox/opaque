...
fn run_test_opaque_index_map_keys_get<K, V>(entries: &[(K, V)])
where
    ...
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    for key in map.keys::<K, V>() {
        assert!(map.get::<K, K, V>(key).is_some());
    }
}

fn run_test_opaque_index_map_keys_get_values<K, V>(generator: oimt::PrefixGenerator<K, V>)
where
    K: Clone + Eq + hash::Hash + fmt::Debug + 'static,
    V: Clone + Eq + fmt::Debug + 'static,
{
    for entries in generator {
        run_test_opaque_index_map_keys_get(entries.as_slice());
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_gen = $range_gen:expr, const_gen = $const_gen:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_opaque_index_map_keys_get_empty() {
                let keys: [$key_typ; 0] = [];
                ...
                run_test_opaque_index_map_keys_get_values(generator);
            }

            #[test]
            fn test_opaque_index_map_keys_get_range_values() {
                let generator = $range_gen;
                run_test_opaque_index_map_keys_get_values(generator);
            }

            #[test]
            fn test_opaque_index_map_keys_get_constant_values() {
