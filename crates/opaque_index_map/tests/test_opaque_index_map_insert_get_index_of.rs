...
*/
fn run_test_opaque_index_map_insert_get_index_of<K, V>(entries: &[(K, V)])
where
    ...
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    for (index, (key, value)) in expected_entries.iter().enumerate() {
        ...
        let result = map.get_index_of::<K, K, V>(key);

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_map_insert_get_index_of_values<K, V>(generator: oimt::PrefixGenerator<K, V>)
where
    K: Clone + Eq + Ord + hash::Hash + fmt::Debug + 'static,
    V: Clone + Eq + fmt::Debug + 'static,
{
    for entries in generator {
        run_test_opaque_index_map_insert_get_index_of(entries.as_slice());
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_gen = $range_gen:expr, const_gen = $const_gen:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_insert_get_index_of_empty() {
                ...
                let generator = oimt::key_value_pairs(keys_vec.iter::<$key_typ>().cloned(), values_vec.iter::<$value_typ>().cloned());
                run_test_opaque_index_map_insert_get_index_of_values(generator);
            }
