...
fn run_test_opaque_index_map_clone<K, V>(entries: &[(K, V)])
where
    ...
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    let expected = map.as_slice::<K, V>();
    let result = cloned_map.as_slice::<K, V>();

    assert_eq!(result, expected);
}

fn run_test_opaque_index_map_clone_values<K, V>(entries: &[(K, V)])
where
    K: Clone + Eq + hash::Hash + fmt::Debug + 'static,
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_clone(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_clone_empty() {
                ...
                run_test_opaque_index_map_clone_values(&entries);
            }
