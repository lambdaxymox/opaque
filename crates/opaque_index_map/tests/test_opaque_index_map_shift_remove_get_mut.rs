...
fn run_test_opaque_index_map_shift_remove_get_mut<K, V>(entries: &[(K, V)])
where
    ...
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for key in keys.iter() {
        let expected = map.get_mut::<K, K, V, hash::RandomState, alloc::Global>(key).cloned();
        let result = map.shift_remove::<K, K, V, hash::RandomState, alloc::Global>(key);

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_map_shift_remove_get_mut_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_shift_remove_get_mut(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_shift_remove_get_mut_empty() {
                ...
                run_test_opaque_index_map_shift_remove_get_mut_values(&entries);
            }
