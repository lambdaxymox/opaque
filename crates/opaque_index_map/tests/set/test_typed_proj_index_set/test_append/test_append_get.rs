...
fn run_test_typed_proj_index_set_append_get_source<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
...
fn run_test_typed_proj_index_set_append_get_destination<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for value in values1.iter() {
        assert!(destination.get(value).is_none());
    }
...
fn run_test_typed_proj_index_set_append_get_source_values<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        ...
        for destination in iterator2 {
            run_test_typed_proj_index_set_append_get_source(
                source,
                destination,
                build_hasher1.clone(),
                alloc1.clone(),
...
fn run_test_typed_proj_index_set_append_get_destination_values<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        let iterator2 = oimt::set::PrefixGenerator::new(values2);
        for destination in iterator2 {
            run_test_typed_proj_index_set_append_get_destination(
                source,
                destination,
                build_hasher1.clone(),
                alloc1.clone(),
...
macro_rules! generate_tests {
    ($module_name:ident, value_type = $value_typ:ty, src_range_spec = $src_range_spec:expr, dst_range_spec = $dst_range_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn run_test_typed_proj_index_set_append_get_source_empty() {
                ...
                run_test_typed_proj_index_set_append_get_source_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }

            #[test]
            fn test_typed_proj_index_set_append_get_source_range_values() {
            ...
            #[test]
            fn run_test_typed_proj_index_set_append_get_destination_empty() {
                let values1: [$value_typ; 0] = [];
                let values2: [$value_typ; 0] = [];
                let build_hasher1 = hash::RandomState::new();
                let alloc1 = alloc::Global;
                let build_hasher2 = hash::RandomState::new();
                let alloc2 = alloc::Global;
                run_test_typed_proj_index_set_append_get_destination_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }

            #[test]
            fn test_typed_proj_index_set_append_get_destination_range_values() {
                let values1 = opaque_index_map_testing::set::range_entries::<$value_typ>($src_range_spec);
                let values2 = opaque_index_map_testing::set::range_entries::<$value_typ>($dst_range_spec);
                let build_hasher1 = hash::RandomState::new();
                let alloc1 = alloc::Global;
