...
fn prop_shift_remove_full_contains<T, S, A>(entries: TypedProjIndexSet<T, S, A>) -> Result<(), TestCaseError>
where
...
fn prop_shift_remove_full_get<T, S, A>(entries: TypedProjIndexSet<T, S, A>) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
...
fn prop_shift_remove_full_get_full<T, S, A>(entries: TypedProjIndexSet<T, S, A>) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
...
fn prop_shift_remove_full_len<T, S, A>(entries: TypedProjIndexSet<T, S, A>) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
...
fn prop_shift_remove_full_preserves_order<T, S, A>(entries: TypedProjIndexSet<T, S, A>) -> Result<(), TestCaseError>
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    fn expected<T, S, A>(set: &TypedProjIndexSet<T, S, A>, index: usize, value: &T) -> Vec<T>
    where
        T: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
        S: any::Any + hash::BuildHasher + Send + Sync + Clone,
        S::Hasher: any::Any + hash::Hasher + Send + Sync,
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_shift_remove_full_contains(entries in super::$set_gen::<$value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    ...
                    super::prop_shift_remove_full_contains(entries)?
                }
                ...
                fn prop_shift_remove_full_len(entries in super::$set_gen::<$value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    let entries: super::TypedProjIndexSet<$value_typ, $build_hasher_typ, $alloc_typ> = entries;
                    super::prop_shift_remove_full_len(entries)?
                }
