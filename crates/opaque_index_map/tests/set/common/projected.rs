...
pub fn strategy_type_projected_index_set_max_len<T, S, A>(
    max_length: usize,
) -> impl Strategy<Value = TypeProjectedIndexSet<T, S, A>>
where
    T: any::Any + Clone + Eq + hash::Hash + Ord + Default + fmt::Debug + Arbitrary + SingleBoundedValue,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone + Default + fmt::Debug,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
...
pub fn strategy_type_projected_index_set_max_len_nonempty<T, S, A>(
    max_length: usize,
) -> impl Strategy<Value = TypeProjectedIndexSet<T, S, A>>
where
    T: any::Any + Clone + Eq + hash::Hash + Ord + Default + fmt::Debug + Arbitrary + SingleBoundedValue,
