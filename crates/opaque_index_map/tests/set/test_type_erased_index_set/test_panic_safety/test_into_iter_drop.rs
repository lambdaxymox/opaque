...
fn run_test_type_erased_index_set_into_iter_take_then_drop<S, A>(length: usize, take_count: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
...
#[test]
fn test_type_erased_index_set_into_iter_drop() {
    let max_length = 128;
...
#[test]
fn test_type_erased_index_set_into_iter_take_then_drop() {
    let max_length = 128;
    let build_hasher = hash::RandomState::default();
    let alloc = alloc::Global;
    for length in 0..max_length {
        for take_count in 0..length {
            run_test_type_erased_index_set_into_iter_take_then_drop(length, take_count, build_hasher.clone(), alloc.clone());
        }
    }
}
