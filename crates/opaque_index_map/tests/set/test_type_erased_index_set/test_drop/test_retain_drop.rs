    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    set.retain::<_, UnhashedValueWrapper<DropCounter>, S, A>(|_| true);
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    set.retain::<_, UnhashedValueWrapper<DropCounter>, S, A>(|_| false);
    let result = drop_counter.drop_count();
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    let (drop_counter, mut set) = create_drop_counter_index_set_in(length, build_hasher, alloc);
    let expected = count_even_indices(set.as_slice::<UnhashedValueWrapper<DropCounter>, S, A>());

    set.retain::<_, UnhashedValueWrapper<DropCounter>, S, A>(|v| v.index % 2 != 0);
    let result = drop_counter.drop_count();
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    let (drop_counter, mut set) = create_drop_counter_index_set_in(length, build_hasher, alloc);
    let expected = count_odd_indices(set.as_slice::<UnhashedValueWrapper<DropCounter>, S, A>());
