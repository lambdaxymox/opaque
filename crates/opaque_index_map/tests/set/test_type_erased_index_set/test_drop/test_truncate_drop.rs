...
fn run_test_type_erased_index_set_truncate_drop_to_zero_direct<S, A>(length: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
...
fn run_test_type_erased_index_set_truncate_drop_to_zero_steps<S, A>(length: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
...
fn run_test_type_erased_index_set_truncate_drop_to_length<S, A>(length: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
...
fn run_test_type_erased_index_set_truncate_drop_to_above_length<S, A>(length: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
...
#[test]
fn test_type_erased_index_set_truncate_drop_to_zero_direct_range() {
    let max_length = 128;
    let build_hasher = hash::RandomState::new();
    let alloc = alloc::Global;
    for length in 0..max_length {
        run_test_type_erased_index_set_truncate_drop_to_zero_direct(length, build_hasher.clone(), alloc.clone());
    }
}

#[test]
fn test_type_erased_index_set_truncate_drop_to_zero_steps_range() {
    let max_length = 128;
    let build_hasher = hash::RandomState::new();
    let alloc = alloc::Global;
    for length in 0..max_length {
        run_test_type_erased_index_set_truncate_drop_to_zero_steps(length, build_hasher.clone(), alloc.clone());
    }
}

#[test]
fn test_type_erased_index_set_truncate_drop_to_length() {
    let max_length = 128;
    let build_hasher = hash::RandomState::new();
    let alloc = alloc::Global;
    for length in 0..max_length {
        run_test_type_erased_index_set_truncate_drop_to_length(length, build_hasher.clone(), alloc.clone());
    }
}

#[test]
fn test_type_erased_index_set_truncate_drop_to_above_length() {
    let max_length = 128;
    let build_hasher = hash::RandomState::new();
    let alloc = alloc::Global;
    for length in 0..max_length {
        run_test_type_erased_index_set_truncate_drop_to_above_length(length, build_hasher.clone(), alloc.clone());
    }
}
