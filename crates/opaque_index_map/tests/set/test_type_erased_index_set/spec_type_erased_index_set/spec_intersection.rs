...
fn prop_intersection_contains<T, S1, S2, A>(entries1: TypeErasedIndexSet, entries2: TypeErasedIndexSet) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S1: any::Any + hash::BuildHasher + Send + Sync + Clone + Default,
    S1::Hasher: any::Any + hash::Hasher + Send + Sync,
...
fn prop_intersection_get_full<T, S1, S2, A>(entries1: TypeErasedIndexSet, entries2: TypeErasedIndexSet) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S1: any::Any + hash::BuildHasher + Send + Sync + Clone + Default,
    S1::Hasher: any::Any + hash::Hasher + Send + Sync,
...
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let lhs = from_intersection_in::<T, S1, S3, A>(
        &from_intersection_in::<T, S1, S2, A>(&entries1, &entries2),
        &entries3,
    );
    let rhs = from_intersection_in::<T, S1, S2, A>(
        &entries1,
...
fn prop_intersection_is_superset<T, S1, S2, A>(entries1: TypeErasedIndexSet, entries2: TypeErasedIndexSet) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S1: any::Any + hash::BuildHasher + Send + Sync + Clone + Default,
    S1::Hasher: any::Any + hash::Hasher + Send + Sync,
...
fn prop_intersection_len<T, S1, S2, A>(entries1: TypeErasedIndexSet, entries2: TypeErasedIndexSet) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S1: any::Any + hash::BuildHasher + Send + Sync + Clone + Default,
    S1::Hasher: any::Any + hash::Hasher + Send + Sync,
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_intersection_len(
                    ...
                    let entries1: super::TypeErasedIndexSet = entries1;
                    let entries2: super::TypeErasedIndexSet = entries2;
                    super::prop_intersection_len::<$value_typ, $build_hasher_typ1, $build_hasher_typ2, $alloc_typ>(entries1, entries2)?
                }
            }
        }
    };
