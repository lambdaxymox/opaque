...
fn prop_swap_remove_index_contains<T, S, A>(entries: TypeErasedIndexSet) -> Result<(), TestCaseError>
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for value in values.iter() {
        ...
        set.swap_remove_index::<T, S, A>(index);
...
fn prop_swap_remove_index_get<T, S, A>(entries: TypeErasedIndexSet) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
...
fn prop_swap_remove_index_len<T, S, A>(entries: TypeErasedIndexSet) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
...
fn prop_swap_remove_index_preserves_order<T, S, A>(entries: TypeErasedIndexSet) -> Result<(), TestCaseError>
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
        ...
        A: any::Any + alloc::Allocator + Send + Sync + Clone,
    {
        ...
        let index = set.get_index_of::<_, T, S, A>(value).unwrap();
        new_set.swap_remove_index::<T, S, A>(index);
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_swap_remove_index_contains(entries in super::$set_gen::<$value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    ...
                    super::prop_swap_remove_index_contains::<$value_typ, $build_hasher_typ, $alloc_typ>(entries)?
                }

                #[test]
                fn prop_swap_remove_index_get(entries in super::$set_gen::<$value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    let entries: super::TypeErasedIndexSet = entries;
                    super::prop_swap_remove_index_get::<$value_typ, $build_hasher_typ, $alloc_typ>(entries)?
                }

                #[test]
                fn prop_swap_remove_index_len(entries in super::$set_gen::<$value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    let entries: super::TypeErasedIndexSet = entries;
                    super::prop_swap_remove_index_len::<$value_typ, $build_hasher_typ, $alloc_typ>(entries)?
                }

                #[test]
                fn prop_swap_remove_index_preserves_order(entries in super::$set_gen::<$value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    let entries: super::TypeErasedIndexSet = entries;
                    super::prop_swap_remove_index_preserves_order::<$value_typ, $build_hasher_typ, $alloc_typ>(entries)?
                }
            }
        }
    };
