...
fn run_test_opaque_index_set_iter_contains<T, S, A>(entries: &[T], build_hasher: S, alloc: A)
where
...
fn run_test_opaque_index_set_iter_contains_values<T, S, A>(entries: &[T], build_hasher: S, alloc: A)
where
    T: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Clone + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for entries in iterator {
        run_test_opaque_index_set_iter_contains(entries, build_hasher.clone(), alloc.clone());
    }
}
    ...
    ($module_name:ident, value_type = $value_typ:ty, range_spec = $range_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_set_iter_contains_empty() {
                ...
                run_test_opaque_index_set_iter_contains_values(&entries, build_hasher, alloc);
            }
