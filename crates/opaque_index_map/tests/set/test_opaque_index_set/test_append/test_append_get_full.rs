...
fn run_test_opaque_index_set_append_get_full_source<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for (index, value) in expected_vec.iter().enumerate() {
        ...
        let result = source.get_full::<_, T, S1, A1>(value);

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_set_append_get_full_destination<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for value in values1.iter() {
        assert!(destination.get_full::<_, T, S2, A2>(value).is_none());
    }
...
fn run_test_opaque_index_set_append_get_full_source_values<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
...
fn run_test_opaque_index_set_append_get_full_destination_values<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
