...
fn run_test_opaque_index_set_append_len_source<T, S1, S2, A>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    build_hasher2: S2,
...
fn run_test_opaque_index_set_append_len_destination<T, S1, S2, A>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    build_hasher2: S2,
...
fn run_test_opaque_index_set_append_len_source_values<T, S1, S2, A>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    build_hasher2: S2,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        ...
        for destination in iterator2 {
            run_test_opaque_index_set_append_len_source(
                source,
                destination,
                build_hasher1.clone(),
                build_hasher2.clone(),
...
fn run_test_opaque_index_set_append_len_destination_values<T, S1, S2, A>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    build_hasher2: S2,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        let iterator2 = oimt::set::PrefixGenerator::new(values2);
        for destination in iterator2 {
            run_test_opaque_index_set_append_len_destination(
                source,
                destination,
                build_hasher1.clone(),
                build_hasher2.clone(),
...
macro_rules! generate_tests {
    ($module_name:ident, value_type = $value_typ:ty, src_range_spec = $src_range_spec:expr, dst_range_spec = $dst_range_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn run_test_opaque_index_set_append_len_source_empty() {
                ...
                run_test_opaque_index_set_append_len_source_values(&values1, &values2, build_hasher1, build_hasher2, alloc);
            }

            #[test]
            fn test_opaque_index_set_append_len_source_range_values() {
            ...
            #[test]
            fn run_test_opaque_index_set_append_len_destination_empty() {
                let values1: [$value_typ; 0] = [];
                let values2: [$value_typ; 0] = [];
                let build_hasher1 = common::opaque_index_set::WrappingBuildHasher1::new(hash::RandomState::new());
                let build_hasher2 = common::opaque_index_set::WrappingBuildHasher2::new(hash::RandomState::new());
                let alloc = alloc::Global;
                run_test_opaque_index_set_append_len_destination_values(&values1, &values2, build_hasher1, build_hasher2, alloc);
            }

            #[test]
            fn test_opaque_index_set_append_len_destination_range_values() {
                let values1 = oimt::set::range_entries::<$value_typ>($src_range_spec);
                let values2 = oimt::set::range_entries::<$value_typ>($dst_range_spec);
                let build_hasher1 = common::opaque_index_set::WrappingBuildHasher1::new(hash::RandomState::new());
                let build_hasher2 = common::opaque_index_set::WrappingBuildHasher2::new(hash::RandomState::new());
