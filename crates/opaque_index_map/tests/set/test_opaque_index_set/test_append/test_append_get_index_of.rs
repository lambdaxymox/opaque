...
fn run_test_opaque_index_set_append_get_index_of_source<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for (index, value) in expected_vec.iter().enumerate() {
        ...
        let result = source.get_index_of::<_, T, S1, A1>(value);

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_set_append_get_index_of_destination<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for value in values1.iter() {
        assert!(destination.get_index_of::<_, T, S2, A2>(value).is_none());
    }
...
fn run_test_opaque_index_set_append_get_index_of_source_values<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
...
fn run_test_opaque_index_set_append_get_index_of_destination_values<T, S1, A1, S2, A2>(
    values1: &[T],
    values2: &[T],
    build_hasher1: S1,
    alloc1: A1,
    ...
    ($module_name:ident, value_type = $value_typ:ty, src_range_spec = $src_range_spec:expr, dst_range_spec = $dst_range_spec:expr) => {
        mod $module_name {
            ...
            fn run_test_opaque_index_set_append_get_index_of_source_empty() {
                ...
                run_test_opaque_index_set_append_get_index_of_source_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }
            ...
            fn run_test_opaque_index_set_append_get_index_of_destination_empty() {
                ...
                let build_hasher2 = hash::RandomState::new();
                let alloc2 = common::opaque_index_set::WrappingAlloc2::new(alloc::Global);
                run_test_opaque_index_set_append_get_index_of_destination_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }
