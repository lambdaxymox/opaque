...
fn strategy_prop_insert_full_preserves_order_new_entry<T, S, A>(max_length: usize) -> impl Strategy<Value = (TypeErasedIndexSet, T)>
where
...
fn prop_insert_full_preserves_order_new_entry<T, S, A>((entries, new_entry): (TypeErasedIndexSet, T)) -> Result<(), TestCaseError>
where
