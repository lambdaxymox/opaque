...
fn run_test_opaque_index_set_insert_full_preserves_order_new_entry<T, S, A>(entries: &[T], build_hasher: S, alloc: A, new_entry: &T)
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    assert!(!set.contains::<T, T, S, A>(new_entry));

    let values_before: Vec<T> = set.iter::<T, S, A>().cloned().collect();

    set.insert_full::<T, S, A>(new_entry.clone());
...
fn run_test_opaque_index_set_insert_full_preserves_order_new_entry_values<T, S, A>(entries: &[T], build_hasher: S, alloc: A, new_entry: &T)
where
    T: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Clone + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for entries in iter {
        run_test_opaque_index_set_insert_full_preserves_order_new_entry(entries, build_hasher.clone(), alloc.clone(), new_entry);
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, value_type = $value_typ:ty, new_entry = $new_entry:expr, range_spec = $range_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_opaque_index_set_insert_full_preserves_order_new_entry_empty() {
                let values: Vec<$value_typ> = Vec::from(&[]);
                let entries = oimt::set::values(values.iter().cloned());
                let build_hasher = hash::RandomState::new();
                let alloc = alloc::Global;
                let new_entry = $new_entry;
                run_test_opaque_index_set_insert_full_preserves_order_new_entry_values(&entries, build_hasher, alloc, &new_entry);
            }

            #[test]
            fn test_opaque_index_set_insert_full_preserves_order_new_entry_range_values() {
                let spec = $range_spec;
