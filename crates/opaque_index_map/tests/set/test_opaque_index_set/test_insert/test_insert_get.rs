...
fn run_test_opaque_index_set_insert_get<T, S, A>(entries: &[T], build_hasher: S, alloc: A)
where
...
fn run_test_opaque_index_set_insert_get_values<T, S, A>(entries: &[T], build_hasher: S, alloc: A)
where
    T: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Clone + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for entries in iterator {
        run_test_opaque_index_set_insert_get(entries, build_hasher.clone(), alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, value_type = $value_typ:ty, range_spec = $range_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_opaque_index_set_insert_get_empty() {
                let values: Vec<$value_typ> = Vec::from(&[]);
                let entries = oimt::set::values(values.iter().cloned());
                let build_hasher = hash::RandomState::new();
                let alloc = alloc::Global;
                run_test_opaque_index_set_insert_get_values(&entries, build_hasher, alloc);
            }

            #[test]
            fn test_opaque_index_set_insert_get_range_values() {
                let spec = $range_spec;
