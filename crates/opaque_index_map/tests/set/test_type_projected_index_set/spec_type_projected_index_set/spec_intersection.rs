...
fn prop_intersection_get<T, S1, S2, A>(
    entries1: TypeProjectedIndexSet<T, S1, A>,
    entries2: TypeProjectedIndexSet<T, S2, A>,
) -> Result<(), TestCaseError>
where
...
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let lhs = from_intersection_in(&from_intersection_in(&entries1, &entries2), &entries3);
    let rhs = from_intersection_in(&entries1, &from_intersection_in(&entries2, &entries3));
...
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let lhs = from_intersection_in(&entries1, &from_union_in(&entries2, &entries3));
...
fn prop_intersection_len1<T, S1, S2, A>(
    entries1: TypeProjectedIndexSet<T, S1, A>,
    entries2: TypeProjectedIndexSet<T, S2, A>,
) -> Result<(), TestCaseError>
where
...
fn prop_intersection_len2<T, S1, S2, A>(
    entries1: TypeProjectedIndexSet<T, S1, A>,
    entries2: TypeProjectedIndexSet<T, S2, A>,
) -> Result<(), TestCaseError>
where
