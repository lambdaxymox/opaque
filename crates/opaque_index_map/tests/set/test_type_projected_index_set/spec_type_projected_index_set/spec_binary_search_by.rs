...
fn prop_binary_search_by_iter<T, S, A>(entries: TypeProjectedIndexSet<T, S, A>) -> Result<(), TestCaseError>
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for value in set.iter() {
        prop_assert!(set.binary_search_by(|v| v.cmp(value)).is_ok());
    }

    Ok(())
}

fn prop_binary_search_by_get_index_of<T, S, A>(entries: TypeProjectedIndexSet<T, S, A>) -> Result<(), TestCaseError>
where
    T: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for value in set.iter() {
        ...
        let result = set.binary_search_by(|v| v.cmp(value));
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_binary_search_by_iter(entries in super::$set_gen::<$value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    ...
                    super::prop_binary_search_by_iter(entries)?
                }
