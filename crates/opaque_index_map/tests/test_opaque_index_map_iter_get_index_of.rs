...
fn run_test_opaque_index_map_iter_get_index_of<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    let map = common::from_entries(&entries);
    for (index, (key, _)) in map.iter::<K, V, hash::RandomState, alloc::Global>().enumerate() {
        let expected = Some(index);
        let result = map.get_index_of::<K, K, V, hash::RandomState, alloc::Global>(key);

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_map_iter_get_index_of_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_iter_get_index_of(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_iter_get_index_of_empty() {
                ...
                run_test_opaque_index_map_iter_get_index_of_values(&entries);
            }
