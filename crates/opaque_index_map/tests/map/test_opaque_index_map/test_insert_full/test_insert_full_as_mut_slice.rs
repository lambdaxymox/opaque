...
fn run_test_opaque_index_map_insert_full_as_mut_slice<K, V, S, A>(entries: &mut [(K, V)], build_hasher: S, alloc: A)
where
...
fn run_test_opaque_index_map_insert_full_as_mut_slice_values<K, V, S, A>(entries: &[(K, V)], build_hasher: S, alloc: A)
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
    S: any::Any + hash::BuildHasher + Clone + Send + Sync + Clone,
