...
fn run_test_opaque_index_map_append_contains_key_source<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for key in values1.iter().map(|(k, v)| k) {
        assert!(source.contains_key::<_, K, V, S1, A1>(key));
    }
...
fn run_test_opaque_index_map_append_contains_key_destination<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for key in values1.iter().map(|(k, v)| k) {
        assert!(!destination.contains_key::<_, K, V, S2, A2>(key));
    }
...
fn run_test_opaque_index_map_append_contains_key_source_values<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        ...
        for destination in iterator2 {
            run_test_opaque_index_map_append_contains_key_source(
                source,
                destination,
                build_hasher1.clone(),
                alloc1.clone(),
...
fn run_test_opaque_index_map_append_contains_key_destination_values<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        let iterator2 = oimt::map::PrefixGenerator::new(values2);
        for destination in iterator2 {
            run_test_opaque_index_map_append_contains_key_destination(
                source,
                destination,
                build_hasher1.clone(),
                alloc1.clone(),
...
macro_rules! generate_tests {
    (
        ...
        mod $module_name {
            ...
            #[test]
            fn run_test_opaque_index_map_append_contains_key_source_empty() {
                ...
                run_test_opaque_index_map_append_contains_key_source_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }

            #[test]
            fn test_opaque_index_map_append_contains_key_source_range_values() {
            ...
            #[test]
            fn run_test_opaque_index_map_append_contains_key_destination_empty() {
                let values1: [($key_typ, $value_typ); 0] = [];
                let values2: [($key_typ, $value_typ); 0] = [];
                let build_hasher1 = hash::RandomState::new();
                let alloc1 = alloc::Global;
                let build_hasher2 = hash::RandomState::new();
                let alloc2 = alloc::Global;
                run_test_opaque_index_map_append_contains_key_destination_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }

            #[test]
            fn test_opaque_index_map_append_contains_key_destination_range_values() {
                let values1 = oimt::map::range_entries::<$key_typ, $value_typ>($src_range_spec);
                let values2 = oimt::map::range_entries::<$key_typ, $value_typ>($dst_range_spec);
                let build_hasher1 = hash::RandomState::new();
                let alloc1 = alloc::Global;
