...
fn run_test_opaque_index_map_append_get_index_of_source<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for (index, (key, _)) in expected_vec.iter().enumerate() {
        ...
        let result = source.get_index_of::<_, K, V, S1, A1>(key);

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_map_append_get_index_of_source_values<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        ...
        for destination in iterator2 {
            run_test_opaque_index_map_append_get_index_of_source(
                source,
                destination,
                build_hasher1.clone(),
                alloc1.clone(),
...
macro_rules! generate_tests {
    (
        ...
        mod $module_name {
            ...
            #[test]
            fn run_test_opaque_index_map_append_get_index_of_source_empty() {
                ...
                run_test_opaque_index_map_append_get_index_of_source_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }
