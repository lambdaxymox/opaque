...
{
    let result: Vec<V> = map.as_slice::<K, V, S, A>().values().cloned().collect();

    result
}

fn run_test_opaque_index_map_insert_as_slice<K, V, S, A>(entries: &[(K, V)], build_hasher: S, alloc: A)
where
...
fn run_test_opaque_index_map_insert_as_slice_values<K, V, S, A>(entries: &[(K, V)], build_hasher: S, alloc: A)
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
    S: any::Any + hash::BuildHasher + Clone + Send + Sync + Clone,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_insert_as_slice(entries, build_hasher.clone(), alloc.clone());
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_insert_as_slice_empty() {
                ...
                run_test_opaque_index_map_insert_as_slice_values(&entries, build_hasher, alloc);
            }

            #[test]
            fn test_opaque_index_map_insert_as_slice_range_values() {
                let spec = $range_spec;
            ...
            #[test]
            fn test_opaque_index_map_insert_as_slice_const_values() {
                let spec = $const_spec;
