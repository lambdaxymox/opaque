...
fn prop_binary_search_by_keys<K, V, S, A>(entries: TypeErasedIndexMap) -> Result<(), TestCaseError>
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for key in map.keys::<K, V, S, A>() {
        prop_assert!(map.binary_search_by::<_, K, V, S, A>(|k, _v| k.cmp(key)).is_ok());
    }

    Ok(())
}

fn prop_binary_search_by_values<K, V, S, A>(entries: TypeErasedIndexMap) -> Result<(), TestCaseError>
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + Ord + fmt::Debug,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for value in map.values::<K, V, S, A>() {
        prop_assert!(map.binary_search_by::<_, K, V, S, A>(|_k, v| v.cmp(value)).is_ok());
    }

    Ok(())
}

fn prop_binary_search_by_get_index_of<K, V, S, A>(entries: TypeErasedIndexMap) -> Result<(), TestCaseError>
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + Ord + fmt::Debug,
    S: any::Any + hash::BuildHasher + Send + Sync + Clone,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for key in map.keys::<K, V, S, A>() {
        ...
        let result = map.binary_search_by::<_, K, V, S, A>(|k, _v| k.cmp(key));
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_binary_search_by_keys(entries in super::$map_gen::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    ...
                    super::prop_binary_search_by_keys::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>(entries)?
                }

                #[test]
                fn prop_binary_search_by_values(entries in super::$map_gen::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    let entries: super::TypeErasedIndexMap = entries;
                    super::prop_binary_search_by_values::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>(entries)?
                }

                #[test]
                fn prop_binary_search_by_get_index_of(entries in super::$map_gen::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    let entries: super::TypeErasedIndexMap = entries;
                    super::prop_binary_search_by_get_index_of::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>(entries)?
                }
            }
        }
    };
