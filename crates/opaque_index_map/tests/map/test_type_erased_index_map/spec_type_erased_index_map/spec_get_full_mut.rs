...
fn from_keys_in<K, V, S, A>(map: &TypeErasedIndexMap) -> TypeProjectedVec<K, A>
where
...
{
    let mut keys = TypeProjectedVec::new_proj_in(map.allocator::<K, V, S, A>().clone());
    for key in map.keys::<K, V, S, A>().cloned() {
        keys.push(key);
    }
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    let keys = from_keys_in::<K, V, S, A>(&map);
    for key in keys.iter() {
        let expected = {
            ...
            let maybe_key_value = map.get_key_value_mut::<_, K, V, S, A>(key).map(|(k, v)| (k.clone(), v.clone()));
        ...
        let result = map.get_full_mut::<_, K, V, S, A>(key).map(|(i, k, v)| (i, k.clone(), v.clone()));
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_insert_get_full_mut_equiv_get_index_of_get_key_value_mut(entries in super::$map_gen::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    ...
                    super::prop_insert_get_full_mut_equiv_get_index_of_get_key_value_mut::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>(entries)?
                }
            }
        }
    };
