...
fn strategy_prop_insert_preserves_order_new_entry<K, V, S, A>(max_length: usize) -> impl Strategy<Value = (TypeProjectedIndexMap<K, V, S, A>, (K, V))>
where
...
fn prop_insert_preserves_order_new_entry<K, V, S, A>((entries, new_entry): (TypeProjectedIndexMap<K, V, S, A>, (K, V))) -> Result<(), TestCaseError>
where
