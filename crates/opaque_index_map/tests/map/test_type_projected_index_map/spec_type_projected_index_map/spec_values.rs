...
fn prop_values_ordering<K, V, S, A>(entries: TypeProjectedIndexMap<K, V, S, A>) -> Result<(), TestCaseError>
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for i in 0..map.len() {
        let expected = map.get_index(i).map(|(_k, v)| v);
        ...
        mod $module_name {
            ...
            proptest! {
                ...
                fn prop_values_ordering(entries in super::$map_gen::<$key_typ, $value_typ, $build_hasher_typ, $alloc_typ>($max_length)) {
                    ...
                    super::prop_values_ordering(entries)?
                }
            }
        }
    };
