...
fn run_test_typed_proj_index_map_swap_remove_entry_contains_key<K, V, S, A>(entries: &[(K, V)], build_hasher: S, alloc: A)
where
...
fn run_test_typed_proj_index_map_swap_remove_entry_contains_key_values<K, V, S, A>(entries: &[(K, V)], build_hasher: S, alloc: A)
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
    S: any::Any + hash::BuildHasher + Clone + Send + Sync + Clone,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for entries in iter {
        run_test_typed_proj_index_map_swap_remove_entry_contains_key(entries, build_hasher.clone(), alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_typed_proj_index_map_swap_remove_entry_contains_key_empty() {
                ...
                run_test_typed_proj_index_map_swap_remove_entry_contains_key_values(&entries, build_hasher, alloc);
            }

            #[test]
            fn test_typed_proj_index_map_swap_remove_entry_contains_key_range_values() {
                let spec = $range_spec;
            ...
            #[test]
            fn test_typed_proj_index_map_swap_remove_entry_contains_key_constant_values() {
                let spec = $const_spec;
