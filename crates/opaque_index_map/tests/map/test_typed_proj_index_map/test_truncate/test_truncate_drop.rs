...
fn run_test_typed_proj_index_map_truncate_drop_to_zero_direct_values<S, A>(max_length: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone + Default,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default,
...
fn run_test_typed_proj_index_map_truncate_drop_to_zero_steps_values<S, A>(max_length: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone + Default,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default,
...
fn run_test_typed_proj_index_map_truncate_drop_to_length_values<S, A>(max_length: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone + Default,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default,
...
fn run_test_typed_proj_index_map_truncate_drop_to_above_length_values<S, A>(max_length: usize, build_hasher: S, alloc: A)
where
    S: any::Any + hash::BuildHasher + Send + Sync + Clone + Default,
    S::Hasher: any::Any + hash::Hasher + Send + Sync,
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default,
...
fn test_typed_proj_index_map_truncate_drop_to_zero_direct_range() {
    ...
    run_test_typed_proj_index_map_truncate_drop_to_zero_direct_values::<hash::RandomState, alloc::Global>(max_length, build_hasher, alloc);
}
...
fn test_typed_proj_index_map_truncate_drop_to_zero_steps_range() {
    ...
    let build_hasher = hash::RandomState::new();
    let alloc = alloc::Global;
    run_test_typed_proj_index_map_truncate_drop_to_zero_steps_values::<hash::RandomState, alloc::Global>(max_length, build_hasher, alloc);
}
...
fn test_typed_proj_index_map_truncate_drop_to_length() {
    ...
    let build_hasher = hash::RandomState::new();
    let alloc = alloc::Global;
    run_test_typed_proj_index_map_truncate_drop_to_length_values::<hash::RandomState, alloc::Global>(max_length, build_hasher, alloc);
}
...
fn test_typed_proj_index_map_truncate_drop_to_above_length() {
    ...
    let build_hasher = hash::RandomState::new();
    let alloc = alloc::Global;
    run_test_typed_proj_index_map_truncate_drop_to_above_length_values::<hash::RandomState, alloc::Global>(max_length, build_hasher, alloc);
}
