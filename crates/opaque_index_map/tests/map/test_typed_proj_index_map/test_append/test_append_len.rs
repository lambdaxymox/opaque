...
fn run_test_typed_proj_index_map_append_len_source<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
...
fn run_test_typed_proj_index_map_append_len_destination<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
...
fn run_test_typed_proj_index_map_append_len_source_values<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        ...
        for destination in iterator2 {
            run_test_typed_proj_index_map_append_len_source(
                source,
                destination,
                build_hasher1.clone(),
                alloc1.clone(),
...
fn run_test_typed_proj_index_map_append_len_destination_values<K, V, S1, A1, S2, A2>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    alloc1: A1,
    ...
    A2: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        let iterator2 = oimt::map::PrefixGenerator::new(values2);
        for destination in iterator2 {
            run_test_typed_proj_index_map_append_len_destination(
                source,
                destination,
                build_hasher1.clone(),
                alloc1.clone(),
...
macro_rules! generate_tests {
    (
        ...
        mod $module_name {
            ...
            #[test]
            fn run_test_typed_proj_index_map_append_len_source_empty() {
                ...
                run_test_typed_proj_index_map_append_len_source_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }
            ...
            #[test]
            fn run_test_typed_proj_index_map_append_len_destination_empty() {
                let values1: [($key_typ, $value_typ); 0] = [];
                let values2: [($key_typ, $value_typ); 0] = [];
                let build_hasher1 = hash::RandomState::new();
                let alloc1 = alloc::Global;
                let build_hasher2 = hash::RandomState::new();
                let alloc2 = alloc::Global;
                run_test_typed_proj_index_map_append_len_destination_values(&values1, &values2, build_hasher1, alloc1, build_hasher2, alloc2);
            }
