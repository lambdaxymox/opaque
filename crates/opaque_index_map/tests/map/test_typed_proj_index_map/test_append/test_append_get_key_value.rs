...
fn run_test_typed_proj_index_map_append_get_source<K, V, S1, S2, A>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    build_hasher2: S2,
...
fn run_test_typed_proj_index_map_append_get_destination<K, V, S1, S2, A>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    build_hasher2: S2,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for (key, _) in values1.iter() {
        assert!(destination.get_key_value(key).is_none());
    }
...
fn run_test_typed_proj_index_map_append_get_source_values<K, V, S1, S2, A>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    build_hasher2: S2,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        ...
        for destination in iterator2 {
            run_test_typed_proj_index_map_append_get_source(
                source,
                destination,
                build_hasher1.clone(),
                build_hasher2.clone(),
...
fn run_test_typed_proj_index_map_append_get_destination_values<K, V, S1, S2, A>(
    values1: &[(K, V)],
    values2: &[(K, V)],
    build_hasher1: S1,
    build_hasher2: S2,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for source in iterator1 {
        let iterator2 = oimt::map::PrefixGenerator::new(values2);
        for destination in iterator2 {
            run_test_typed_proj_index_map_append_get_destination(
                source,
                destination,
                build_hasher1.clone(),
                build_hasher2.clone(),
...
macro_rules! generate_tests {
    (
        ...
        mod $module_name {
            ...
            #[test]
            fn run_test_typed_proj_index_map_append_get_source_empty() {
                ...
                run_test_typed_proj_index_map_append_get_source_values(&values1, &values2, build_hasher1, build_hasher2, alloc);
            }

            #[test]
            fn test_typed_proj_index_map_append_get_source_range_values() {
            ...
            #[test]
            fn run_test_typed_proj_index_map_append_get_destination_empty() {
                let values1: [($key_typ, $value_typ); 0] = [];
                let values2: [($key_typ, $value_typ); 0] = [];
                let build_hasher1 = common::typed_proj_index_map::WrappingBuildHasher1::new(hash::RandomState::new());
                let build_hasher2 = common::typed_proj_index_map::WrappingBuildHasher2::new(hash::RandomState::new());
                let alloc = alloc::Global;
                run_test_typed_proj_index_map_append_get_destination_values(&values1, &values2, build_hasher1, build_hasher2, alloc);
            }

            #[test]
            fn test_typed_proj_index_map_append_get_destination_range_values() {
                let values1 = oimt::map::range_entries::<$key_typ, $value_typ>($src_range_spec);
                let values2 = oimt::map::range_entries::<$key_typ, $value_typ>($dst_range_spec);
                let build_hasher1 = common::typed_proj_index_map::WrappingBuildHasher1::new(hash::RandomState::new());
                let build_hasher2 = common::typed_proj_index_map::WrappingBuildHasher2::new(hash::RandomState::new());
