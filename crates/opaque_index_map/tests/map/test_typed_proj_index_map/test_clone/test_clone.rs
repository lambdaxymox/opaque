...
fn run_test_typed_proj_index_map_clone<K, V, S, A>(entries: &[(K, V)], build_hasher: S, alloc: A)
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    let expected = map.as_slice();
    let result = cloned_map.as_slice();

    assert_eq!(result, expected);
}

fn run_test_typed_proj_index_map_clone_values<K, V, S, A>(entries: &[(K, V)], build_hasher: S, alloc: A)
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
    S: any::Any + hash::BuildHasher + Clone + Send + Sync + Clone,
    ...
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for entries in iter {
        run_test_typed_proj_index_map_clone(entries, build_hasher.clone(), alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_typed_proj_index_map_clone_empty() {
                ...
                run_test_typed_proj_index_map_clone_values(&entries, build_hasher, alloc);
            }

            #[test]
            fn test_typed_proj_index_map_clone_range_values() {
                let spec = $range_spec;
            ...
            #[test]
            fn test_typed_proj_index_map_clone_const_values() {
                let spec = $const_spec;
