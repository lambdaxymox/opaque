...
fn run_test_opaque_index_map_insert_get_full_values<K, V>(generator: oimt::PrefixGenerator<K, V>)
where
    K: Clone + Eq + Ord + hash::Hash + fmt::Debug + 'static,
    V: Clone + Eq + fmt::Debug + 'static,
{
    for entries in generator {
        run_test_opaque_index_map_insert_get_full(entries.as_slice());
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_gen = $range_gen:expr, const_gen = $const_gen:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_insert_get_full_empty() {
                ...
                run_test_opaque_index_map_insert_get_full_values(generator);
            }
