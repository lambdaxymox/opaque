...
fn run_test_opaque_index_map_iter_get_index<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    let map = common::from_entries(&entries);
    for (index, (key, value)) in map.iter::<K, V, hash::RandomState, alloc::Global>().enumerate() {
        let expected = Some((key.clone(), value.clone()));
        let result = map.get_index::<K, V, hash::RandomState, alloc::Global>(index).map(|(k, v)| (k.clone(), v.clone()));

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_map_iter_get_index_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_iter_get_index(entries);
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_opaque_index_map_iter_get_index_empty() {
                let keys: Vec<$key_typ> = Vec::from(&[]);
                let values: Vec<$value_typ> = Vec::from(&[]);
                let entries = oimt::key_value_pairs(keys.iter().cloned(), values.iter().cloned());
                run_test_opaque_index_map_iter_get_index_values(&entries);
            }

            #[test]
            fn test_opaque_index_map_iter_get_index_range_values() {
                let spec = $range_spec;
            ...
            #[test]
            fn test_opaque_index_map_iter_get_index_constant_values() {
                let spec = $const_spec;
