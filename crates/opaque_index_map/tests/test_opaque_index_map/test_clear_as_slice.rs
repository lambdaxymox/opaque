...
fn run_test_opaque_index_map_clear_as_slice<K, V>(entries: &[(K, V)])
where
    ...
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    assert_eq!(result.as_slice::<K, V, hash::RandomState, alloc::Global>(), expected.as_slice::<K, V,  hash::RandomState, alloc::Global>());
}

fn run_test_opaque_index_map_clear_as_slice_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_clear_as_slice(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_clear_as_slice_empty() {
                ...
                run_test_opaque_index_map_clear_as_slice_values(&entries);
            }
