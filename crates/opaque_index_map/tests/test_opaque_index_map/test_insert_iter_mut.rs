...
fn result<K, V>(map: &mut OpaqueIndexMap) -> Vec<(K, V)>
where
    K: any::Any + Clone + Eq + hash::Hash,
    V: any::Any + Clone + Eq,
{
    let result: Vec<(K, V)> = map
        .iter_mut::<K, V, hash::RandomState, alloc::Global>()
...
fn run_test_opaque_index_map_insert_iter_mut<K, V>(entries: &[(K, V)])
where
...
fn run_test_opaque_index_map_insert_iter_mut_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_insert_iter_mut(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_insert_iter_mut_empty() {
                ...
                run_test_opaque_index_map_insert_iter_mut_values(&entries);
            }
