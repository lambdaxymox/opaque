...
fn run_test_opaque_index_map_swap_remove_get_key_value<K, V>(entries: &[(K, V)])
where
    ...
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for key in keys.iter() {
        ...
        let result = {
            let maybe_value = map.swap_remove::<K, K, V, hash::RandomState, alloc::Global>(key);
...
fn run_test_opaque_index_map_swap_remove_get_key_value_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_swap_remove_get_key_value(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_swap_remove_get_key_value_empty() {
                ...
                run_test_opaque_index_map_swap_remove_get_key_value_values(&entries);
            }
