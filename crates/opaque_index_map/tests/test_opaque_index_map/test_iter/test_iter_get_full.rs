...
fn run_test_opaque_index_map_iter_get_full<K, V>(entries: &[(K, V)])
where
    ...
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for (key, value) in map.iter::<K, V, hash::RandomState, alloc::Global>() {
        ...
        let result = map.get_full::<K, K, V, hash::RandomState, alloc::Global>(key).map(|(i, k, v)| (k.clone(), v.clone()));

        assert_eq!(result, expected);
    }
}

fn run_test_opaque_index_map_iter_get_full_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_iter_get_full(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_iter_get_full_empty() {
                ...
                run_test_opaque_index_map_iter_get_full_values(&entries);
            }
