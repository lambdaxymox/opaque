...
fn run_test_opaque_index_map_swap_remove_len<K, V>(entries: &[(K, V)])
where
    ...
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for (i, key_i) in keys.iter().enumerate() {
        map.swap_remove::<K, K, V, hash::RandomState, alloc::Global>(key_i);
...
fn run_test_opaque_index_map_swap_remove_len_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_swap_remove_len(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_swap_remove_len_empty() {
                ...
                run_test_opaque_index_map_swap_remove_len_values(&entries);
            }
