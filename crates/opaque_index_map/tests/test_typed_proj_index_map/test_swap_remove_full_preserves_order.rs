...
fn run_test_opaque_index_map_swap_remove_full_preserves_order<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
...
fn run_test_opaque_index_map_swap_remove_full_preserves_order_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_swap_remove_full_preserves_order(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_swap_remove_full_preserves_order_empty() {
                ...
                run_test_opaque_index_map_swap_remove_full_preserves_order_values(&entries);
            }
