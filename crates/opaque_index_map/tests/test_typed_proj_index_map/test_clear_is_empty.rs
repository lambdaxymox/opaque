...
fn run_test_opaque_index_map_clear_is_empty<K, V>(entries: &[(K, V)])
where
    ...
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_clear_is_empty(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_clear_is_empty_empty() {
                ...
                run_test_opaque_index_map_clear_is_empty_values(&entries);
            }
