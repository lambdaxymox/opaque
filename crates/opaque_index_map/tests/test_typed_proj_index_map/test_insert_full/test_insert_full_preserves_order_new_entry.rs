...
fn run_test_typed_proj_index_map_insert_full_preserves_order_new_entry<K, V>(entries: &[(K, V)], new_entry: &(K, V))
where
...
fn run_test_typed_proj_index_map_insert_full_preserves_order_new_entry_values<K, V>(entries: &[(K, V)], new_entry: &(K, V))
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_typed_proj_index_map_insert_full_preserves_order_new_entry(entries, new_entry);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, new_entry = $new_entry:expr, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_typed_proj_index_map_insert_full_preserves_order_new_entry_empty() {
                ...
                run_test_typed_proj_index_map_insert_full_preserves_order_new_entry_values(&entries, &new_entry);
            }
