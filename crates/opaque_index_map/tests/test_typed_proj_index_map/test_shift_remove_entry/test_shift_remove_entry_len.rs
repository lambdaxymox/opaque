...
fn run_test_typed_proj_index_map_shift_remove_entry_len<K, V>(entries: &[(K, V)])
where
...
fn run_test_typed_proj_index_map_shift_remove_entry_len_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_typed_proj_index_map_shift_remove_entry_len(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_typed_proj_index_map_shift_remove_entry_len_empty() {
                ...
                run_test_typed_proj_index_map_shift_remove_entry_len_values(&entries);
            }
