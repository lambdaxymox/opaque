...
fn result<K, V>(map: &TypedProjIndexMap<K, V, hash::RandomState, alloc::Global>) -> Vec<V>
where
    K: any::Any + Clone + Eq + hash::Hash,
    V: any::Any + Clone + Eq,
{
    let result: Vec<V> = map.as_slice().values().cloned().collect();

    result
}

fn run_test_typed_proj_index_map_insert_as_slice<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    let map = common::typed_proj_index_map::from_entries::<K, V>(entries);
...
fn run_test_typed_proj_index_map_insert_as_slice_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_typed_proj_index_map_insert_as_slice(entries);
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_typed_proj_index_map_insert_as_slice_empty() {
                let keys: Vec<$key_typ> = Vec::from(&[]);
                let values: Vec<$value_typ> = Vec::from(&[]);
                let entries = oimt::key_value_pairs(keys.iter().cloned(), values.iter().cloned());
                run_test_typed_proj_index_map_insert_as_slice_values(&entries);
            }

            #[test]
            fn test_typed_proj_index_map_insert_as_slice_range_values() {
                let spec = $range_spec;
            ...
            #[test]
            fn test_typed_proj_index_map_insert_as_slice_const_values() {
                let spec = $const_spec;
