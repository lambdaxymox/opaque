...
fn run_test_opaque_index_map_insert_contains_key_values<K, V>(entries: &[(K, V)])
where
    ...
    V: Clone + Eq + fmt::Debug + 'static,
{
    for len in 0..entries.len() {
        ...
        run_test_opaque_index_map_insert_contains_key(prefix_entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, key_range = $key_range:expr, value_range = $value_range:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_contains_key_empty() {
                ...
                run_test_opaque_index_map_insert_contains_key_values(entries.as_slice::<($key_typ, $value_typ)>());
            }
