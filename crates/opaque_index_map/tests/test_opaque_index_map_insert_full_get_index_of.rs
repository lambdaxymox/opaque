...
fn run_test_opaque_index_map_insert_full_get_index_of<K, V>(entries: &[(K, V)])
where
...
fn run_test_opaque_index_map_insert_full_get_index_of_values<K, V>(entries: &[(K, V)])
where
    K: any::Any + Clone + Eq + Ord + hash::Hash + fmt::Debug,
    V: any::Any + Clone + Eq + fmt::Debug,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_insert_full_get_index_of(&entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_insert_full_get_index_of_empty() {
                ...
                run_test_opaque_index_map_insert_full_get_index_of_values(&entries);
            }
