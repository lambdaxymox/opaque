...
fn run_test_opaque_index_map_keys_get_full<K, V>(entries: &[(K, V)])
where
    ...
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    for key in map.keys::<K, V>() {
        assert!(map.get_full::<K, K, V>(key).is_some());
    }
}

fn run_test_opaque_index_map_keys_get_full_values<K, V>(entries: &[(K, V)])
where
    K: Clone + Eq + hash::Hash + fmt::Debug + 'static,
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    for entries in iter {
        run_test_opaque_index_map_keys_get_full(entries);
    }
}
    ...
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_spec = $range_spec:expr, const_spec = $const_spec:expr) => {
        mod $module_name {
            ...
            fn test_opaque_index_map_keys_get_full_empty() {
                ...
                run_test_opaque_index_map_keys_get_full_values(&entries);
            }
