...
fn result<K, V>(map: &OpaqueIndexMap) -> OpaqueVec
where
    K: Clone + Eq + hash::Hash + 'static,
    V: Clone + Eq + 'static,
{
    ...
    V: Clone + Eq + fmt::Debug + 'static,
{
    ...
    assert_eq!(result.as_slice::<V>(), expected.as_slice::<V>());
}

fn run_test_opaque_index_map_insert_as_slice_values<K, V>(generator: opaque_testing::PrefixGenerator<K, V>)
where
    K: Clone + Eq + hash::Hash + fmt::Debug + 'static,
    V: Clone + Eq + fmt::Debug + 'static,
{
    for entries in generator {
        run_test_opaque_index_map_insert_as_slice(entries.as_slice());
    }
}
...
macro_rules! generate_tests {
    ($module_name:ident, key_type = $key_typ:ty, value_type = $value_typ:ty, range_gen = $range_gen:expr, const_gen = $const_gen:expr) => {
        mod $module_name {
            ...
            #[test]
            fn test_opaque_index_map_as_slice_empty() {
                let keys: [$key_typ; 0] = [];
                ...
                run_test_opaque_index_map_insert_as_slice_values(generator);
            }

            #[test]
            fn test_opaque_index_map_as_slice_range_values() {
                let generator = $range_gen;
                run_test_opaque_index_map_insert_as_slice_values(generator);
            }

            #[test]
            fn test_opaque_index_map_as_slice_const_values() {
