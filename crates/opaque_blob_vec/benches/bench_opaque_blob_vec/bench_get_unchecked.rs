    ...
    c.bench_function("opaque_blob_vec_get_unchecked", |b| {
        b.iter(|| {
            for i in 0..opaque_blob_vec.len() {
                let _ = core::hint::black_box(opaque_blob_vec.get_unchecked::<alloc::Global>(i));
            }
        });
    });
}

criterion_group!(bench_get_unchecked, bench_opaque_blob_vec_get_unchecked, bench_vec_get_unchecked, bench_vec_get);
