...
fn run_test_opaque_blob_vec_shift_remove_forget_unchecked_len<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let values_vec = common::opaque_blob_vec::from_slice_in(values, alloc);

    for i in 0..values.len() {
        let result_vec = {
            let mut vec = common::opaque_blob_vec::clone::<T, A>(&values_vec);
            let values = unsafe {
                let ptr = vec.shift_remove_forget_unchecked::<A>(i).cast::<T>();
                ptr.read()
            };
...
fn run_test_opaque_blob_vec_shift_remove_forget_unchecked_len_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iter {
        run_test_opaque_blob_vec_shift_remove_forget_unchecked_len(slice, alloc.clone());
    }
}
    ...
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            fn test_opaque_blob_vec_shift_remove_forget_unchecked_len_alternating_values() {
                ...
                run_test_opaque_blob_vec_shift_remove_forget_unchecked_len_values(&values, alloc);
            }
        }
    };
}
