...
fn run_test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end<T, A>(values: &[T], alloc: A)
where
    ...
    A: any::Any + alloc::Allocator + Clone,
{
    ...
    for _ in 0..opaque_blob_vec.len() {
        ...
        let result = unsafe {
            let ptr = opaque_blob_vec.shift_remove_forget_unchecked::<A>(last_index).cast::<T>();
            ptr.read()
        };
...
fn run_test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Clone,
{
    ...
    for slice in iter {
        run_test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end_alternating_values() {
                let values = opaque_vec_testing::alternating_values::<$typ, $max_array_size>($alt_spec);
                let alloc = alloc::Global;
                run_test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end_values(&values, alloc);
            }
        }
    };
}
