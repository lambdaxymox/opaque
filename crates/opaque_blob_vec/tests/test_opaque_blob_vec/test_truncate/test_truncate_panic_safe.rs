...
fn test_truncate_on_panic_drop_count() {
    ...
    let mut vec = common::opaque_blob_vec::new_in::<PanicCell<()>, alloc::Global>(alloc::Global);
    unsafe {
        let value = ManuallyDrop::new(triggering_panic_cell.clone());
        let value_ptr = NonNull::new_unchecked(&*value as *const PanicCell<()> as *mut PanicCell<()>).cast::<u8>();
        vec.push::<alloc::Global>(value_ptr);
    }

    assert_eq!(triggering_panic_cell.drop_count(), 0);

    let result = panic::catch_unwind(AssertUnwindSafe(|| {
        vec.truncate::<alloc::Global>(0);
    }));
