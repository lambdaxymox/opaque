...
fn run_test_opaque_blob_vec_push_capacity<T, A>(values: &[T], alloc: A)
where
...
fn run_test_opaque_blob_vec_push_capacity_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iter {
        run_test_opaque_blob_vec_push_capacity(slice, alloc.clone());
    }
}
    ...
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            fn test_opaque_blob_vec_push_capacity_alternating_values() {
                ...
                run_test_opaque_blob_vec_push_capacity_values(&values, alloc);
            }
        }
    };
}
