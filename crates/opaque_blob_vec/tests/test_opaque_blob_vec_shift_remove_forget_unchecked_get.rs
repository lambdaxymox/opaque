...
fn run_test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end<T>(values: &[T])
where
...
fn run_test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end_values<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
    for len in 0..values.len() {
        let prefix_values = &values[0..len];
        run_test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end(prefix_values);
    }
}
    ...
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            fn test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end_alternating_values() {
                ...
                run_test_opaque_blob_vec_shift_remove_forget_unchecked_get_from_end_values(&values);
            }
        }
    };
}
