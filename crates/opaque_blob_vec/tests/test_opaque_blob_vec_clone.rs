...
fn run_test_opaque_blob_vec_clone_occupy_disjoint_memory_locations<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
...
fn run_test_opaque_blob_vec_clone_occupy_disjoint_memory_regions<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
    let vec1 = common::from_typed_slice(values);
...
fn run_test_opaque_blob_vec_clone_values<T>(values: &[T])
where
...
fn run_test_opaque_blob_vec_clone_occupy_disjoint_memory_locations_values<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + TryFrom<usize> + 'static,
    <T as TryFrom<usize>>::Error: fmt::Debug,
{
...
fn run_test_opaque_blob_vec_clone_occupy_disjoint_memory_regions_values<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + TryFrom<usize> + 'static,
    <T as TryFrom<usize>>::Error: fmt::Debug,
{
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_opaque_blob_vec_clone_empty() {
                let values: [$typ; 0] = [];

                run_test_opaque_blob_vec_clone(&values);
            }
