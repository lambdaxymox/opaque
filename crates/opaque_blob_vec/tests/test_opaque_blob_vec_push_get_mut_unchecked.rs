...
fn run_test_opaque_blob_vec_push_get_mut<T, A>(values: &[T], alloc: A)
where
...
fn run_test_opaque_blob_vec_push_get_mut_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: alloc::Allocator + any::Any + Clone,
{
    ...
    for slice in iter {
        run_test_opaque_blob_vec_push_get_mut(slice, alloc.clone());
    }
}
    ...
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            fn test_opaque_blob_vec_push_get_alternating_values() {
                ...
                run_test_opaque_blob_vec_push_get_mut_values(&values, alloc);
            }
        }
    };
}
