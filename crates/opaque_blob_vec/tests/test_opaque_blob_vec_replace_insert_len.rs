...
fn run_test_opaque_blob_vec_replace_insert_len<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
...
fn run_test_opaque_blob_vec_replace_insert_len_values<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
    for len in 0..values.len() {
        let prefix_values = &values[0..len];
        run_test_opaque_blob_vec_replace_insert_len(prefix_values);
    }
}
    ...
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            fn test_opaque_blob_vec_replace_insert_get_unchecked_alternating_values() {
                ...
                run_test_opaque_blob_vec_replace_insert_len_values(&values);
            }
        }
    };
}
