...
fn run_test_opaque_blob_vec_replace_insert_get_unchecked<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
...
fn run_test_opaque_blob_vec_replace_insert_get_unchecked_values<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
    ...
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            fn test_opaque_blob_vec_replace_insert_get_unchecked_alternating_values() {
                ...
                run_test_opaque_blob_vec_replace_insert_get_unchecked_values(&values);
            }
        }
    };
}
