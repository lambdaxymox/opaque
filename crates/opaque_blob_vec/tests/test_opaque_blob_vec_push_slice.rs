...
fn run_test_opaque_blob_vec_push_slice<T>(values: &[T])
where
...
fn run_test_opaque_blob_vec_push_slice_values<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
    for len in 0..values.len() {
        let prefix_values = &values[0..len];
        run_test_opaque_blob_vec_push_slice(prefix_values);
    }
}
    ...
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            fn test_opaque_blob_vec_push_slice_alternating_values() {
                ...
                run_test_opaque_blob_vec_push_slice_values(&values);
            }
        }
    };
}
