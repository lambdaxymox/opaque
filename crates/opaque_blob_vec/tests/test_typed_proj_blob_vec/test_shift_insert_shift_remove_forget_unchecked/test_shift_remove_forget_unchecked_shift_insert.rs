...
fn run_test_typed_proj_blob_vec_shift_remove_forget_unchecked_shift_insert<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for i in 0..values.len() {
        ...
        result.shift_insert(i, removed_value_ptr);

        assert_eq!(common::typed_proj_blob_vec::as_slice::<T, A>(&result), common::typed_proj_blob_vec::as_slice::<T, A>(&expected));
    }
}

fn run_test_typed_proj_blob_vec_shift_remove_forget_unchecked_shift_insert_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iter {
        run_test_typed_proj_blob_vec_shift_remove_forget_unchecked_shift_insert(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_test_typed_proj_blob_vec_shift_remove_forget_unchecked_shift_insert_alternating_values() {
                let values = opaque_vec_testing::alternating_values::<$typ, $max_array_size>($alt_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_blob_vec_shift_remove_forget_unchecked_shift_insert_values(&values, alloc);
            }
        }
    };
}
