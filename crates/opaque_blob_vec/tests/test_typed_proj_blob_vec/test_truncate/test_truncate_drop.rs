...
fn create_drop_counter_blob_vec_in<A>(len: usize, alloc: A) -> (DropCounter, TypedProjBlobVec<A>)
where
    A: any::Any + alloc::Allocator + Send + Sync + Clone + Default,
{
    let drop_counter = DropCounter::new(Rc::new(RefCell::new(0)));
    let mut vec = common::typed_proj_blob_vec::with_capacity_in::<DropCounter, A>(len, alloc);
    for i in 0..len {
        let ptr = unsafe {
