...
fn run_test_typed_proj_blob_vec_replace_insert_slice<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Clone,
{
...
fn run_test_typed_proj_blob_vec_replace_insert_slice_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Clone,
{
    ...
    for slice in iter {
        run_test_typed_proj_blob_vec_replace_insert_slice(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_typed_proj_blob_vec_replace_insert_slice_alternating_values() {
                let values = opaque_vec_testing::alternating_values::<$typ, $max_array_size>($alt_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_blob_vec_replace_insert_slice_values(&values, alloc);
            }
        }
    };
}
