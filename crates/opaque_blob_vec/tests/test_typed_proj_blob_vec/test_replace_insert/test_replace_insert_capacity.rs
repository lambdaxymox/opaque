...
fn run_test_typed_proj_blob_vec_replace_insert_capacity<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    let proj_blob_vec = common::typed_proj_blob_vec::from_slice_in(values, alloc);

    assert!(proj_blob_vec.len() <= proj_blob_vec.capacity());
}

fn run_test_typed_proj_blob_vec_replace_insert_capacity_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iter {
        run_test_typed_proj_blob_vec_replace_insert_capacity(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_typed_proj_blob_vec_replace_insert_capacity_alternating_values() {
                let values = opaque_vec_testing::alternating_values::<$typ, $max_array_size>($alt_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_blob_vec_replace_insert_capacity_values(&values, alloc);
            }
        }
    };
}
