...
fn run_test_typed_proj_blob_vec_shift_insert_slice_end<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
...
fn run_test_typed_proj_blob_vec_shift_insert_slice_end_values<T, A>(values: &[T], alloc: A)
where
    T: any::Any + PartialEq + Clone + fmt::Debug,
    A: any::Any + alloc::Allocator + Send + Sync + Clone,
{
    ...
    for slice in iter {
        run_test_typed_proj_blob_vec_shift_insert_slice_end(slice, alloc.clone());
    }
}
...
macro_rules! generate_tests {
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            #[test]
            fn test_typed_proj_blob_vec_shift_insert_slice_end_alternating_values() {
                let values = opaque_vec_testing::alternating_values::<$typ, $max_array_size>($alt_spec);
                let alloc = alloc::Global;
                run_test_typed_proj_blob_vec_shift_insert_slice_end_values(&values, alloc);
            }
        }
    };
}
