...
fn run_test_opaque_blob_vec_shift_remove_forget_unchecked_end_values<T>(values: &[T])
where
    T: PartialEq + Clone + fmt::Debug + 'static,
{
    ...
    ($typ:ident, $max_array_size:expr, $alt_spec:expr) => {
        mod $typ {
            ...
            fn test_opaque_blob_vec_shift_remove_forget_unchecked_end_alternating_values() {
                ...
                run_test_opaque_blob_vec_shift_remove_forget_unchecked_end_values(&values);
            }
        }
    };
}
