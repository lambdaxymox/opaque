...
#[cfg(feature = "nightly")]
fn run_test_opaque_alloc_allocate_size_with_layout<A>(opaque_alloc: TypeErasedAlloc, layout: alloc::Layout)
where
...
fn run_test_opaque_alloc_allocate_size_with_size_align<A>(alloc: A, size: usize, align: usize)
where
    A: any::Any + alloc::Allocator + Send + Sync,
{
    ...
    run_test_opaque_alloc_allocate_size_with_layout::<A>(opaque_alloc, layout);
}
...
fn test_opaque_alloc_allocate_align_large() {
    ...
    for size in sizes.iter().copied() {
        for align in alignments.iter().copied() {
            run_test_opaque_alloc_allocate_size_with_size_align(alloc, size, align);
        }
    }
}
