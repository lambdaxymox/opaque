...
#[cfg(feature = "nightly")]
fn run_test_typed_proj_alloc_allocate_size_with_layout<A>(proj_alloc: TypeProjectedAlloc<A>, layout: alloc::Layout)
where
    ...
    A: any::Any + alloc::Allocator + Send + Sync,
{
    ...
    run_test_typed_proj_alloc_allocate_size_with_layout::<A>(proj_alloc, layout);
}
...
fn test_typed_proj_alloc_allocate_align_large() {
    ...
    for size in sizes.iter().copied() {
        for align in alignments.iter().copied() {
            run_test_typed_proj_alloc_allocate_size_with_size_align(alloc, size, align);
        }
    }
}
