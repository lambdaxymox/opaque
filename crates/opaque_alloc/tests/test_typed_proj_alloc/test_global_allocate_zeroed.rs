...
#[cfg(feature = "nightly")]
fn run_test_typed_proj_alloc_allocate_zeroed_with_layout<A>(proj_alloc: TypeProjectedAlloc<A>, layout: alloc::Layout)
where
...
where
    A: any::Any + alloc::Allocator + Send + Sync,
{
    let typed_proj_alloc = TypeProjectedAlloc::new(alloc);
    let layout = alloc::Layout::from_size_align(size, align).expect(&format!(
        "Failed to construct layout with size `{:?}` and alignment `{:?}`",
        size, align
    ));

    run_test_typed_proj_alloc_allocate_zeroed_with_layout::<A>(typed_proj_alloc, layout);
}

#[test]
fn test_typed_proj_alloc_allocate_zeroed_large() {
    ...
    for size in sizes.iter().copied() {
        for align in alignments.iter().copied() {
            run_test_typed_proj_alloc_allocate_zeroed_with_size_align(alloc, size, align);
        }
    }
}

#[test]
fn test_typed_proj_alloc_allocate_zeroed_small() {
    let alloc = alloc::Global;
